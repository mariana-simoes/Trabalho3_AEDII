...
int main(void)
{
    ...
    /* --- 30 buscas 'ano > limiar' via BST --- */
    double tempos[N_BUSCAS];

    printf("\n--- Questao 4: Busca 'ano > X' via BST ---\n");
    for (int i = 0; i < N_BUSCAS; i++) {
        double ini = tempoAtual();
        resLista *res = buscarRangeAno(raiz, LIMIARES[i], OP_MAIOR);
        tempos[i] = tempoAtual() - ini;

        printf("[BST] ano > %-4d : %6d registros  %.9f s\n",
               LIMIARES[i], contarResLista(res), tempos[i]);
        liberaResLista(res);
    }

    /* --- Estatísticas --- */
    double med = media(tempos, N_BUSCAS);
    double dp  = desvioPadrao(tempos, N_BUSCAS, med);

    printf("\n========================================\n");
    printf("   RESULTADO — Questao 4 (BST por 'ano')\n");
    printf("========================================\n");
    printf("  Media       : %.9f s\n", med);
    printf("  Desvio pad. : %.9f s\n", dp);
    printf("  Tempo total : %.6f s\n",  soma(tempos, N_BUSCAS));
    printf("========================================\n");
